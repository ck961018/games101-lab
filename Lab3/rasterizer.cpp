...
void rst::rasterizer::rasterize_triangle(
  ...
  auto v = t.toVector4();
  auto min_x = std::floor(std::min(std::min(v[0].x(), v[1].x()), v[2].x()));
  auto min_y = std::floor(std::min(std::min(v[0].y(), v[1].y()), v[2].y()));
  auto max_x = std::ceil(std::max(std::max(v[0].x(), v[1].x()), v[2].x()));
  auto max_y = std::ceil(std::max(std::max(v[0].y(), v[1].y()), v[2].y()));

  float step[] = {.25f, .25f, .75f, .75f, .25f};

  for (auto x = min_x; x <= max_x; x++) {
    for (auto y = min_y; y <= max_y; y++) {
      auto index = get_index(x, y) * 4;
      for (size_t i = 0; i < 4; i++) {
        auto xx = static_cast<float>(x) + step[i],
             yy = static_cast<float>(y) + step[i + 1];
        if (insideTriangle(xx, yy, t.v)) {
          ...
          if (super_depth_buf[index + i] > zp) {
            ...
            super_depth_buf[index + i] = zp;
            super_frame_buf[index + i] = fragment_shader(payload);
          }
        }
      }
      auto pixel_color =
          super_frame_buf[index + 0] + super_frame_buf[index + 1] +
          super_frame_buf[index + 2] + super_frame_buf[index + 3];
      set_pixel({x, y}, pixel_color / 4.0);
      // if (insideTriangle(x + .5f, y + .5f, t.v)) {
      //   auto [alpha, beta, gamma] = computeBarycentric2D(x + .5f, y + .5f,
      //   t.v); float Z = 1.0 / (alpha / v[0].w() + beta / v[1].w() + gamma /
      //   v[2].w()); float zp = alpha * v[0].z() / v[0].w() + beta * v[1].z() /
      //   v[1].w() +
      //              gamma * v[2].z() / v[2].w();
      //   zp *= Z;

      //   if (super_depth_buf[get_index(x, y)] > zp) {
      ...
      //     super_depth_buf[get_index(x, y)] = zp;
...
void rst::rasterizer::clear(rst::Buffers buff) {
  if ((buff & rst::Buffers::Color) == rst::Buffers::Color) {
    ...
    std::fill(super_frame_buf.begin(), super_frame_buf.end(),
              Eigen::Vector3f{0, 0, 0});
  }
  if ((buff & rst::Buffers::Depth) == rst::Buffers::Depth) {
    std::fill(super_depth_buf.begin(), super_depth_buf.end(),
