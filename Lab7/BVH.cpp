...
BVHAccel::BVHAccel(std::vector<Object *> p, int maxPrimsInNode,
  ...
  if (splitMethod == SplitMethod::NAIVE) {
    ...
    printf("\rBVH-SAH Generation complete: \nTime Taken: %i hrs, %i mins, %i "
           "secs\n\n",
           hrs, mins, secs);
  }
}
  ...
  if (objects.size() == 1) {
    ...
    if (splitMethod == SplitMethod::NAIVE) {
      ...
      for (size_t i = 1; i < len; i++) {
        ...
        auto surface_inv = 1.0 / bounds.SurfaceArea();

        auto t = leftBounds.SurfaceArea() * surface_inv * i +
                 rightBounds.SurfaceArea() * surface_inv * (len - i) + 0.125;
        if (t < minT) {
          minT = t;
          minI = i;
        }
      }
      middling = beginning + minI;
    }
